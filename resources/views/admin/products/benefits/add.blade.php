...
@section('content')
    <div class="container px-6 mx-auto grid">
        ...
        <div class="px-4 py-3 mb-8 bg-white rounded-lg shadow-md dark:bg-gray-800">
            <form action="{{ route('admin.store.benefit', $benefit->id) }}" method="post" enctype="multipart/form-data">
                @csrf
                <label class="block text-sm">
                    <span class="text-gray-700 dark:text-gray-400">
                        Benefit Name
                    </span>
                    <input name="name" value="{{ old('name', $benefit->name) }}" type="text"
                        ...
                        placeholder="Enter Opinion Name" />
                ...
                <label class="block text-sm">
                    ...
                    <select name="opinion"
                        ...
                        @foreach ($opinions as $opinion)
                            <option {{ $opinion->id === $benefit->opinion_id ? 'selected' : '' }} value="{{ $opinion->id }}">
                                {{ $opinion->name }}</option>
                        @endforeach
                    </select>
                </label>
                <x-input-error :messages="$errors->get('opinion')" class="mt-2" />
                </label>
                <button type="submit"
